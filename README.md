...
| Test file | `tests/flix-validation.spec.ts` |
...
**Code location:** `tests/flix-validation.spec.ts`, lines **240–331**
...
3. **`pages/FlixPage.ts`** — added `loadFullPageTopToBottom()` (gradual full-page scroll
...
```powershell
# Run the suite with full artifacts
npx playwright test tests/flix-validation.spec.ts --reporter=html,list
